...
void insertar(NODO &arbol,int x,NODO padre01);
...
int cant=0;
int raizdata;
void niveles34(NODO arbol0,int ant);
int main(){
	...
	do{
		...
		switch(op){
			...
			case 7:{
				int f;
				cout<<"inserte el dato a eliminar"<<endl;
				cin>>f;
				eliminar(raiz,f);
				system("pause");
				break;
...
void insertar(NODO &arbol,int x,NODO padre01){
	if(arbol==NULL){
		...
		if(x<valorR){
			...
			insertar(arbol->izquierdo,x,arbol);
		}
		else{
			
			insertar(arbol->derecho,x,arbol);
		}
	}
...
void imprimir(int bus){
	...
	nodo *asa =raiz;
	niveles34(asa,xd);
	//alchile debe dar lo mismo osea 5-2-3 entonces no lo se crack;	
	//lo da al reves, pero esta bien (correguir si es que tengo ganas xd)
}
void niveles34(NODO arbol0,int ant){
	int ayuda=ant;
	if(arbol0==NULL){
	...
	if(arbol0->data==ant){
		...
		}while(arbol0->owl!=NULL);	
		cout<<arbol0->data;
	}
	else{
		niveles34(arbol0->izquierdo,ayuda);
		niveles34(arbol0->derecho,ayuda);
	}
}
